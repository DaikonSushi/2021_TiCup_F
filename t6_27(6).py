...
# 循迹部分，停车功能写在这里
def track_line(stop_num,turn_num):
    global turn_count
    while True:
        TrackSensorLeftValue1 = GPIO.input(Tracking_Left1)
        TrackSensorLeftValue2 = GPIO.input(Tracking_Left2)
        ...
        if turn_count == stop_num:
            ...
            time.sleep(1)
            weight(1)
            send.send_signal_once(host)
            turn_180()
            turn_count += 1
            continue 
        elif turn_count == turn_num:
            runrun(1.5,70)
            turn_180()
            car.Car_Run(45,45)
            time.sleep(0.01)
            car.Car_Stop()
            time.sleep(1)
            listen.listen_signal_once(host)
            turn_count += 1
            continue
        else:
...
try:
    ...
    while True : #这是一个死循环，会每次回到起点，打开摄像头，识别代码，按照代码规划路径，走到最后结束
        ...
        if(num==1):
            ...
            all_directions = [f,f,l,l,r,f,l,f,f,f]#最后一个随便写的，不会走到
            stop_num = 5
            turn_num = 8
        elif(num==6):
